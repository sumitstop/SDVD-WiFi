...
class parse_mac_reset_impl : public parse_mac_reset {

public:

parse_mac_reset_impl(bool log, bool debug, int reset_flag) :
		block("parse_mac_reset",
				gr::io_signature::make(0, 0, 0),
				gr::io_signature::make(0, 0, 0)),
		d_log(log), d_last_seq_no(-1), d_total_packets(0),
		d_debug(debug), d_reset_flag(reset_flag) {

	message_port_register_in(pmt::mp("in"));
	set_msg_handler(pmt::mp("in"), boost::bind(&parse_mac_reset_impl::parse, this, _1));

	message_port_register_out(pmt::mp("fer"));
	message_port_register_out(pmt::mp("tpr"));
}

~parse_mac_reset_impl() {
...
void parse_data(char *buf, int length) {
	...
	d_total_packets += 1;
        std::cout << "RF" << d_reset_flag << std::endl;
...
parse_mac_reset::sptr
parse_mac_reset::make(bool log, bool debug, int reset_flag) {
	return gnuradio::get_initial_sptr(new parse_mac_reset_impl(log, debug, reset_flag));
}
